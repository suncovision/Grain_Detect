           ...
           bool view_img)
{
    ...
    for (size_t i = 0; i < images.size(); i += 1) {
        ...
        for (size_t j = i; j < i + 1 && j < images.size(); j++)
        {
            ...
            std::cout << "result_info size: " << result_info.size() << std::endl;

            cv::putText(img, "grain numbers: " + std::to_string(result_info.size()), cv::Point(0, 50), 1, 3, cv::Scalar(0, 255, 0), 3);

            std::string route = "C:\\Users\\Lenovo\\Desktop\\9\\";
            cv::String filename = route + "" + std::to_string(i) + ".bmp";
